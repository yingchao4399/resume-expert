...
- 两阶段快速分析：JD 需求地图、事实匹配与岗位概览；支持实时心跳、主动取消和 3 分钟硬上限
...
3. 填写目标岗位和 JD 后开始快速分析；首次只阻塞完成 JD 需求地图、事实匹配和岗位概览，不提前生成面试策略或简历优化内容。
4. 查看 JD、诊断与 ATS 就绪度。
5. 在“经历补证”中补充真实经历，生成的内容会先进入待确认证据；需要面试准备时进入对应页面按需生成。
6. 进入“制作”阶段，明确点击“生成优化方案”，选择优化风格后再生成最终简历；面试策略未生成不会阻塞制作。
...
模型返回无法解析或结构不合法时，系统只自动修复一次；再次失败会返回明确错误，不会把不完整数据写入简历。首次岗位分析整体最多运行 3 分钟、最多调用 Provider 10 次，单次结构请求最多 60 秒；JD 每批最多 16 条、匹配每批最多 12 条。取消、超时、截断重试耗尽或流意外中断都不会写入半成品。

## 数据与隐私

- 简历、投递和复盘保存在当前浏览器的 `localStorage` 中；项目化经历事实保存在独立 IndexedDB，均不会自动同步到云端。
- V4 JSON 备份会异步汇总 localStorage 与 IndexedDB，并支持合并或替换恢复。
