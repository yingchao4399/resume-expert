...
- 三阶段深度分析：JD 需求解析、经历事实匹配与逐条面试策略；支持实时进度、主动取消和 6 分钟硬上限
- Mock / 真实大模型两种运行模式
...
- AI 设置支持保存前连接测试、15 秒测试超时和可操作的错误分类；一般正式请求 120 秒超时，首次岗位分析中的单次请求限制为 90 秒
...
3. 填写目标岗位和 JD 后开始分析；首次分析只执行 JD、事实匹配和面试策略，不提前生成简历优化内容。
4. 查看 JD、诊断与 ATS 就绪度。
5. 在“经历补证”中补充真实经历，生成的内容会先进入待确认证据。
6. 进入“制作”阶段，明确点击“生成优化方案”，选择优化风格后再生成最终简历。
...
模型返回无法解析或结构不合法时，系统只自动修复一次；再次失败会返回明确错误，不会把不完整数据写入简历。首次岗位分析整体最多运行 6 分钟、最多调用 Provider 10 次，单次模型请求最多 90 秒；取消、超时、截断重试耗尽或流意外中断都不会写入半成品。
