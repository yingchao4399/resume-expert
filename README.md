...
当前版本：`v1.9.7`。本维护版本修复长 JD 导致面试策略失败、备份/刷新误判，以及简历导入引用与待确认流程的数据完整性问题。
...
7. 进入“制作”阶段，明确点击“生成优化方案”，选择优化风格后再生成最终简历；面试策略未生成不会阻塞制作。
8. 在“最终简历”中人工编辑、检查来源记录并统一模板排版。
9. 在“投递与进展”中记录状态、下一步日期和实际使用的简历。
10. 在“导出结果”中下载 DOCX，或打开打印页保存为 PDF；面试后可在独立复盘模块保存诊断。
...
- V7 JSON 备份会异步汇总 localStorage 与 IndexedDB，并保存完整导入资料和待确认片段，继续兼容 V1–V6。
...
本版使用业务文档 Schema 10、文档库 Schema 11、备份 V7。旧导入数据会迁移为待复核的完整资料；重新确认后才会进入证据库和后续 AI 上下文。
