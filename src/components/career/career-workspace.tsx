...
import { AlertCircle, Check, ChevronDown, ChevronUp, Loader2, Pencil, Plus, Save, Settings2, Trash2 } from "lucide-react";
...
function Interview({ snapshot, persist }: PaneProps) {
  ...
  const [retry, setRetry] = useState<{ current: CareerInterviewSession | null; endRequested: boolean } | null>(null);
  const run = async (current: CareerInterviewSession | null, endRequested = false) => {
    setBusy(true); setError(""); setRetry({ current, endRequested });
    try {
      ...
      await persist({ ...snapshot, interviewSessions: [...snapshot.interviewSessions.filter((item) => item.id !== id), next] }); setSessionId(id); setAnswers({}); setRetry(null);
  ...
  return <div className="space-y-4">{!session && <Card><CardContent className="space-y-3 p-4"><Field label="目标岗位"><Input value={draft.targetRole} onChange={(e) => setDraft({ ...draft, targetRole: e.target.value })} /></Field><Field label="经历/项目名称"><Input value={draft.experienceTitle} onChange={(e) => setDraft({ ...draft, experienceTitle: e.target.value })} /></Field><Field label="现有背景与已知事实"><Textarea className="min-h-32" value={draft.background} onChange={(e) => setDraft({ ...draft, background: e.target.value })} /></Field><Button disabled={busy || !draft.targetRole.trim() || !draft.experienceTitle.trim() || draft.background.trim().length < 10} onClick={() => void run(null)}>{busy && <Loader2 className="animate-spin" />}开始项目梳理</Button></CardContent></Card>}
    ...
    <div className="space-y-3">{session.latestTurn?.claimDrafts.map((claim) => <Card key={claim.id} className={claim.status === "needs-review" ? "border-amber-300" : ""}><CardContent className="flex justify-between gap-4 p-4"><div><div className="flex gap-2"><Badge variant="outline">{kindLabel(claim.kind)}</Badge><Badge variant={claim.status === "candidate" ? "secondary" : "danger"}>{claim.status === "candidate" ? "候选" : "待复核"}</Badge></div><p className="mt-2 text-sm">{claim.text}</p><p className="mt-2 text-xs text-neutral-500">原文：“{claim.sourceQuote || "无可验证引用"}”</p></div><Button size="sm" onClick={() => void acceptClaim(claim.id)}><Check />{claim.status === "candidate" ? "确认事实" : "保存待复核"}</Button></CardContent></Card>)}</div></>}{error && <div className="rounded-lg border border-red-300 bg-red-50 p-4 text-sm text-red-800" role="alert" aria-live="assertive"><div className="flex items-start gap-2"><AlertCircle className="mt-0.5 h-4 w-4 shrink-0" /><div><p className="font-medium">项目梳理未写入任何数据</p><p className="mt-1 leading-6">{error}</p><p className="mt-1 text-xs">请刷新模型清单并测试连接；若模型仍不兼容，可手动选择推荐模型或 Mock 后重试。</p></div></div><div className="mt-3 flex gap-2">{retry && <Button size="sm" variant="outline" disabled={busy} onClick={() => void run(retry.current, retry.endRequested)}>{busy && <Loader2 className="animate-spin" />}重新尝试</Button>}<Button size="sm" variant="outline" onClick={() => window.dispatchEvent(new Event("resume-expert-open-ai-settings"))}><Settings2 />打开 AI 设置</Button></div></div>}</div>;
}
