...
export function ResumeBackupDialog({ open, onOpenChange }: ResumeBackupDialogProps) {
  ...
  const { documents, activeDocumentId, importDocuments } = useResumeStore();
  ...
  const finishImport = (mode: "merge" | "replace") => {
    if (!pending) return;
    importDocuments(pending.documents, mode);
    setPending(null);
    onOpenChange(false);
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <Button variant="outline" disabled={!active} onClick={() => active && downloadResumeBackup([active], "resume-expert-current.json")}>
            <Download className="h-4 w-4" /> 导出当前版本
          </Button>
          <Button variant="outline" onClick={() => downloadResumeBackup(documents)}>
        ...
        {pending && (
          <div className="space-y-3 rounded-md border bg-neutral-50 p-3 text-sm">
            <p>已验证 {pending.documents.length} 份简历，导出时间：{new Date(pending.exportedAt).toLocaleString("zh-CN")}</p>
