...
export function ResumeBackupDialog({ open, onOpenChange }: ResumeBackupDialogProps) {
  ...
  const finishImport = async (mode: "merge" | "replace") => {
    ...
    try {
      ...
      try { importDocuments(prepared.documents, mode, prepared.careerEvidence, pending.jobApplications, pending.interviewReviews, true); }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          <Button variant="outline" disabled={!active} onClick={() => active && void downloadResumeBackupV8([active], careerEvidence.filter((item) => item.sourceDocumentId === null || item.sourceDocumentId === active.id), jobApplications.filter((item) => item.resumeDocumentId === active.id), interviewReviews.filter((item) => item.resumeDocumentId === active.id), "resume-expert-current.json", "current")}>
            <Download className="h-4 w-4" /> 导出当前版本
          </Button>
          <Button variant="outline" onClick={() => void downloadResumeBackupV8(documents, careerEvidence, jobApplications, interviewReviews)}>
