    ...
    <div>
      ...
      <Tabs defaultValue="requirements">
        ...
        <TabsContent value="summary" className="space-y-4">
          {jdAnalysisDocument.expertSummary && <Card><CardHeader><CardTitle className="text-sm">岗位专家结论</CardTitle></CardHeader><CardContent className="grid gap-3 text-sm sm:grid-cols-2"><div><p className="text-xs font-medium text-neutral-500">岗位使命</p><p className="mt-1">{jdAnalysisDocument.expertSummary.mission}</p></div><div><p className="text-xs font-medium text-neutral-500">工作重心</p><p className="mt-1">{jdAnalysisDocument.expertSummary.workFocus.join("；") || "信息不足"}</p></div><div><p className="text-xs font-medium text-neutral-500">核心产出</p><p className="mt-1">{jdAnalysisDocument.expertSummary.coreOutcomes.join("；") || "尚未确认"}</p></div><div><p className="text-xs font-medium text-neutral-500">高价值未知</p><p className="mt-1">{jdAnalysisDocument.expertSummary.highValueUnknowns.join("；") || "暂无"}</p></div></CardContent></Card>}
          ...
          {groups.map(group => <details key={group.id} className="rounded-lg border bg-white p-4" open={editingId && group.requirementIds.includes(editingId) ? true : undefined}>
            ...
            <Card key={requirement.id} className={requirement.reviewStatus === "rejected" ? "opacity-60" : ""}>
              <CardContent className="p-4">
                ...
                ) : <><p className="mt-2 text-sm font-medium">{requirement.normalizedText}</p><div className="mt-2 grid gap-2 text-xs text-neutral-600 sm:grid-cols-2"><p><span className="font-medium text-neutral-500">岗位专家拆解：</span>{requirement.actionVerb ? `${requirement.actionVerb} ${requirement.objectText ?? ""}` : requirement.normalizedText}</p><p><span className="font-medium text-neutral-500">需要证明：</span>{requirement.requiredEvidenceTypes?.join("、") || "场景、行动与结果"}{requirement.numericConstraints?.length ? ` · 条件：${requirement.numericConstraints.join("、")}` : ""}</p></div></>}
