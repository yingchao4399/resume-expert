...
export const PROMPT_REGISTRY: PromptDefinition[] = [
  definition("resume.deep-jd", "深度 JD 解析", "把 JD 原始条目拆成可追溯的原子岗位要求。", "岗位分析", "analyze", "deep-jd-v1", ["input", "jobTargetContext", "sourceItems"], "deep_jd_requirement_map", [resumeSystem, prompts("src/lib/ai/jd-prompts.ts", "buildDeepJDPrompt"), schema(aiSchemaSource, "createDeepJDModelResultSchema"), policy()], "buildDeepJDPrompt({{input}}, {{jobTargetContext}}, {{sourceItems}})", ["eval:jd", "eval:mock"], { maxTokens: 12000 }),
  definition("resume.requirement-match", "要求—事实匹配", "将已校验的岗位要求与最多 12 条已确认经历事实匹配。", "简历诊断", "analyze", "requirement-match-v1", ["input", "jobTargetContext", "requirements", "careerClaims"], "requirement_fact_match", [resumeSystem, prompts("src/lib/ai/jd-prompts.ts", "buildRequirementMatchPrompt"), schema(aiSchemaSource, "createDiagnosisMatchResultSchema"), policy()], "buildRequirementMatchPrompt({{input}}, {{jobTargetContext}}, {{requirements}}, {{careerClaims}})", ["eval:jd", "eval:mock"], { maxTokens: 12000 }),
  definition("resume.interview-strategy", "逐条面试策略", "围绕岗位要求生成验证方向、回答结构和反向提问。", "面试准备", "analyze", "interview-strategy-v1", ["input", "jobTargetContext", "requirements", "matches", "clarificationNeeds"], "requirement_interview_strategy", [resumeSystem, prompts("src/lib/ai/jd-prompts.ts", "buildRequirementInterviewPrompt"), schema(aiSchemaSource, "createInterviewPrepResultSchema"), policy()], "buildRequirementInterviewPrompt({{input}}, {{jobTargetContext}}, {{requirements}}, {{matches}}, {{clarificationNeeds}})", ["eval:jd", "eval:mock"], { maxTokens: 12000 }),
  definition("resume.analysis-output", "分析后的简历优化", "在岗位分析完成后生成优化项和结构化简历草稿。", "简历诊断", "analyze", "analysis-output-v1", ["input", "optimizeStyle", "coreSummary"], "resume_optimized_output", [resumeSystem, prompts("src/lib/ai/prompts.ts", "buildAnalyzeOutputPrompt"), schema(aiSchemaSource, "optimizeResumeResultSchema"), policy()], "buildAnalyzeOutputPrompt({{input}}, {{optimizeStyle}}, {{coreSummary}})", ["eval:mock", "eval:ai"], { maxTokens: 12000 }),
  ...
  definition("resume.optimize-items", "重新生成优化项", "按照用户选择的风格重新生成简历优化建议。", "简历制作", "optimize", "optimize-items-v1", ["input", "style"], "resume_optimized_items", [resumeSystem, prompts("src/lib/ai/prompts.ts", "buildOptimizeUserPrompt"), schema(aiSchemaSource, "optimizedItemsResultSchema"), policy()], "buildOptimizeUserPrompt({{input}}, {{style}})", ["eval:mock", "eval:ai"], { temperature: 0.5, maxTokens: 4000 }),
  ...
  definition("resume.finalize", "最终简历生成", "汇总材料、优化项和已确认补证生成最终简历。", "简历制作", "finalize", "finalize-v1", ["input", "style", "optimizedItems", "followUpQuestions"], "resume_final_document", [resumeSystem, prompts("src/lib/ai/prompts.ts", "buildFinalizeResumePrompt"), schema(aiSchemaSource, "finalResumeResultSchema"), policy()], "buildFinalizeResumePrompt({{input}}, {{style}}, {{optimizedItems}}, {{followUpQuestions}})", ["eval:mock", "eval:ai"], { temperature: 0.2, maxTokens: 5000 }),
