...
import type { FinalResume, PdfGenerationProgress, ResumeLayoutConfig } from "@/types/resume";
...
export async function generateATSTextPdf(
  ...
  const baseSize = model.layout.baseFontSize;
  const lineHeight = baseSize * model.layout.lineHeight;
  ...
  const requireSpace = (height: number) => { if (y - height < margin) addPage(); };

  const nameSize = baseSize + 12;
  ...
  if (model.contactLine) {
    const contactSize = Math.max(8, baseSize - 1);
  ...
  for (let index = 0; index < model.blocks.length; index += 1) {
    ...
    if (block.kind === "section-heading") {
      y -= Math.max(5, model.layout.sectionSpacing * 0.55);
      const headingSize = baseSize + 1;
...
export async function generateVisualPdf(
  ...
  options: Pick<PdfGenerationOptions, "onProgress"> = {},
): Promise<Uint8Array> {
...
export async function downloadVisualPdf(pageElements: HTMLElement[], resume: FinalResume, targetRole: string, options: Pick<PdfGenerationOptions, "onProgress"> = {}) {
